...
public class SmartFileUserTimer {
    ...
    public static void firstIn() {
        if (!SmartFileSPUtils.getBoolean("firstin_app_start", false)) {
            SmartFileSPUtils.putBoolean("firstin_app_start", true);
            SmartFileSPUtils.putLong("firstin_app_start_time", System.currentTimeMillis());
        }

    }

    public static long getFirstInTime() {
        return SmartFileSPUtils.getLong("firstin_app_start_time", 0L);
    }
