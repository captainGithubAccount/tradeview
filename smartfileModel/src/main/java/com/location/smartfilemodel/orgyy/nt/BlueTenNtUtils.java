...
public class SmartFileNtUtils {

    public SmartFileNtUtils() {
    }

    public static boolean isNotificationEnabled() {
        boolean isEnable = true;
        if (VERSION.SDK_INT >= 26) {
            if (isEnableV26(SmartFileOrgManager.mContext)) {
                isEnable = true;
            } else {
                isEnable = false;
            }
    ...
    public static boolean isOngoingServiceRunning(Class serviceClass) {
        ActivityManager manager = (ActivityManager) SmartFileOrgManager.mContext.getSystemService(Context.ACTIVITY_SERVICE);
