package com.location.smartfilemodel;
...
import com.location.smartfilemodel.orgyy.SmartFileUserUtils;
import com.location.smartfilemodel.orgyy.msg.SmartFileMsgUploader;
import com.location.smartfilemodel.orgyy.nt.SmartFileNtCountUtil;
import com.location.smartfilemodel.orgyy.nt.SmartFileNtFgService;
import com.location.smartfilemodel.shownotificy.SmartFileNtSender;
...
public class SmartFileOrgManager {
    ...
    public static final SmartFileOrgManager INSTANCE = new SmartFileOrgManager();
    ...
    public static void showSceneNotify(int notifyId, PendingIntent pendingIntent, RemoteViews remoteViewsBig, RemoteViews remoteViewsMid, RemoteViews remoteViewsMini, boolean isSilent, boolean isIgnoreLastPushTime) {
        SmartFileNtSender.showSceneNtOrg9hz(notifyId, pendingIntent, remoteViewsBig, remoteViewsMid, remoteViewsMini, isSilent, isIgnoreLastPushTime);
    }
    ...
    final class AppLifeCycleCallBack implements Application.ActivityLifecycleCallbacks {
        ...
        public void onActivityCreated(@NotNull Activity activity, @org.jetbrains.annotations.Nullable Bundle bundle) {
            SmartFileSPUtils.putLong("lastActivityOnPause", 0L);
            SmartFileOrgManager.INSTANCE.setCurrentActivity(activity);
            SmartFileOrgManager.creatingActivities.add(activity);
            SmartFileOrgManager.livingActivities.add(activity);
            SmartFileOrgManager.INSTANCE.setPaused(false);
        }

        public void onActivityStarted(@NotNull Activity activity) {
            SmartFileOrgManager.visibleActivities.add(activity);
        }
        ...
        public void onActivityResumed(@NotNull Activity activity) {
            SmartFileSPUtils.putLong("lastActivityOnPause", 0L);
            SmartFileOrgManager.INSTANCE.setCurrentActivity(activity);
            SmartFileOrgManager.creatingActivities.remove(activity);
            SmartFileOrgManager.INSTANCE.setPaused(false);
        }

        public void onActivityPaused(@NotNull Activity activity) {
            SmartFileOrgManager.INSTANCE.setPaused(true);
            SmartFileOrgManager.INSTANCE.setPausedTime(System.currentTimeMillis());
            SmartFileOrgManager.creatingActivities.remove(activity);
        }

        public void onActivityStopped(@NotNull Activity activity) {
            SmartFileOrgManager.visibleActivities.remove(activity);
            SmartFileOrgManager.creatingActivities.remove(activity);
        }

        public void onActivityDestroyed(@NotNull Activity activity) {
            if (SmartFileOrgManager.INSTANCE.isCurrActivity(activity)) {
                SmartFileOrgManager.INSTANCE.setCurrentActivity((Activity) null);
            }
            SmartFileOrgManager.creatingActivities.remove(activity);
            SmartFileOrgManager.livingActivities.remove(activity);
        }
