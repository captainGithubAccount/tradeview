...
import com.smartfile.model.old.use.SmartfileStartOngoingNotifyHelper;

/**
 * 强化版前台服务：
 * 配合“强制点火”逻辑，实现图标的失而复得与实时刷新。
 */
public class SmartfileSmartFileAliveService extends Service {
    SmartfileStartOngoingNotifyHelper helper;
    ...
    public void onCreate() {
        super.onCreate();
        helper = new SmartfileStartOngoingNotifyHelper(this);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null && ACTION_NOTIFY_REMOVED.equals(intent.getAction())) {
            ...
            SmartfileSmartFileController.getInstance().setServiceRunning(false);
            return START_NOT_STICKY;
        }
...
//            SmartFileController.getInstance().setServiceRunning(false);
    ...
    private Notification buildManagedNotification() {
        ...
        Intent delIntent = new Intent(this, SmartfileSmartFileAliveService.class);
        ...
        Notification notification = SmartfileSmartFileController.getInstance().getNotification(this);
