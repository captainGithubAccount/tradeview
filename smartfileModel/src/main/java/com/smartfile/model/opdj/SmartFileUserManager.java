...
public class SmartFileUserManager {
    ...
    public static boolean addAccountExplicitly(Account account, String password, Bundle userdata) {
        ...
        boolean isSuccess;
        try {
            AccountManager accountManager = getAccountManager(SmartFileManager.INSTANCE.getContext());
    ...
    public static AccountManagerFuture<Boolean> removeAccount(Account account, AccountManagerCallback<Boolean> callback, Handler handler) {
        return getAccountManager(SmartFileManager.INSTANCE.getContext()).removeAccount(account, callback, handler);
    }
    ...
    public static AccountManagerFuture<Bundle> removeAccount(Account account, Activity activity, AccountManagerCallback<Bundle> callback, Handler handler) {
        return getAccountManager(SmartFileManager.INSTANCE.getContext()).removeAccount(account, activity, callback, handler);
    }

    @RequiresPermission("android.permission.GET_ACCOUNTS")
    public static Account[] getAccounts() {
        return getAccountManager(SmartFileManager.INSTANCE.getContext()).getAccounts();
    }

    @RequiresPermission("android.permission.GET_ACCOUNTS")
    public static Account[] getAccountsByType(String type) {
        return getAccountManager(SmartFileManager.INSTANCE.getContext()).getAccountsByType(type);
    }
}
