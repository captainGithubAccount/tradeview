...
import com.smartfile.model.SpeedFirebaseEventUtils;
import com.smartfile.model.SpeedManager;
import com.smartfile.model.change.SpeedChangeUtils;
import com.smartfile.model.opdj.nt.SpeedNtCancelFgService;
...
public class SpeedNtSender {
    ...
    public static boolean showSceneNtOrg9hz(int notifyId, PendingIntent pendingIntent, RemoteViews remoteViewsBig, RemoteViews remoteViewsMid, RemoteViews remoteViewsMini, boolean isSilent, boolean isIgnoreLastPushTime, SpeedChangeUtils.NoticeType noticeType) {
        ...
        int smallIcon = R.mipmap.speed_logo;
        ...
        SpeedManager.handler.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                SpeedFirebaseEventUtils.INSTANCE.setAnalyticsEvent("noti_touch_show_count", "", SpeedManager.mContext);
    ...
    private static boolean doCycleSend(NotificationManagerCompat notificationManager, int id, Notification notification) {
        ...
        if (!SpeedManager.INSTANCE.isForeground() && !SpeedManager.INSTANCE.hasCreatingActivity()) {
            ...
            if (isNotificationEnabled && screenOn) {
                ...
                SpeedFirebaseEventUtils.INSTANCE.setAnalyticsEvent("noti_touch_show_error", "", SpeedManager.mContext);
                return false;
            }
        } else {
            if (SpeedManager.isDebug) {
