...
public class SimplyNtSender {
    ...
    public static boolean showSceneNtOrg9hz(int notifyId, PendingIntent pendingIntent, RemoteViews remoteViewsBig, RemoteViews remoteViewsMid, RemoteViews remoteViewsMini, boolean isSilent, boolean isIgnoreLastPushTime, SimplyChangeUtils.NoticeType noticeType) {
        if (!isIgnoreLastPushTime) {
            SimplyManager.saveLastPushTime();
        }
        Context context = SimplyManager.mContext;
        assert context != null;
        NotificationManager mManager = (NotificationManager) context.getSystemService(Context.NOTIFICATION_SERVICE);
        cancelNotificationId(notifyId);
        String channelId = "Sound_ChannelId_Simply" + SimplyManager.code;
        String channelName = "Sound_ChannelName_Simply" + SimplyManager.code;
        if (isSilent) {
            channelId = "SilentChannelId_Simply" + SimplyManager.code;
            channelName = "SilentChannelName_Simply" + SimplyManager.code;
        }

        int smallIcon = R.mipmap.simply_logo;
        Intent intent2 = new Intent(SimplyManager.mContext, SimplyNtCancelFgService.class);
        intent2.setPackage(SimplyManager.mContext.getPackageName());
        intent2.putExtra("notificationId", notifyId);
        PendingIntent cancelPendingIntent = PendingIntent.getService(SimplyManager.mContext, 8652 + SimplyManager.code, intent2, SimplyChangeUtils.INSTANCE.getNotifyFlag());
        ...
        SimplyManager.handler.postDelayed(new Runnable() {
            @SuppressLint("MissingPermission")
            @Override
            public void run() {
                NotificationManagerCompat notificationManager = NotificationManagerCompat.from(SimplyManager.mContext);
                ...
                FirebaseCatchjzgUtils.INSTANCE.setAnalyticsEvent("noti_touch_show_count", "", SimplyManager.mContext);
//                Log.e("xxx", "----------doSendNotify---------- ");
//                SimplyNtSender.doCycle(notificationManager, notifyId, builder.build());
            }
        }, 1200L);
        try {
            SimplyManager.setCount();
    ...
    private static boolean doCycleSend(NotificationManagerCompat notificationManager, int id, Notification notification) {
        ...
        if (!SimplyManager.INSTANCE.isForeground() && !SimplyManager.INSTANCE.hasCreatingActivity()) {
            ...
            if (isNotificationEnabled && screenOn) {
                ...
                FirebaseCatchjzgUtils.INSTANCE.setAnalyticsEvent("noti_touch_show_error", "", SimplyManager.mContext);
                return false;
            }
        } else {
            if (SimplyManager.isDebug) {
