...
public class BlueTenNtUtils {

    public BlueTenNtUtils() {
    }

    public static boolean isNotificationEnabled() {
        boolean isEnable = true;
        if (VERSION.SDK_INT >= 26) {
            if (isEnableV26(BlueTenOrgManager.mContext)) {
                isEnable = true;
            } else {
                isEnable = false;
            }
    ...
    public static boolean isOngoingServiceRunning(Class serviceClass) {
        ActivityManager manager = (ActivityManager) BlueTenOrgManager.mContext.getSystemService(Context.ACTIVITY_SERVICE);
