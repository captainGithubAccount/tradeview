...
public class WhiterUserTimer {

    public WhiterUserTimer() {
    }

    public static void firstIn() {
        if (!WhiterSPUtils.getBoolean("firstin_app_start", false)) {
            WhiterSPUtils.putBoolean("firstin_app_start", true);
            WhiterSPUtils.putLong("firstin_app_start_time", System.currentTimeMillis());
        }

    }

    public static long getFirstInTime() {
        return WhiterSPUtils.getLong("firstin_app_start_time", 0L);
    }
