...
public class Whiter1Service extends Service {
    ...
    public static void tryStartLaunchMainService() {
        if (isLiving) {
            ...
            if (!isStartService && System.currentTimeMillis() - lastTime > 10000L) {
                ...
                startService(WhiterManager.mContext);
            }
        }
    }
    ...
    public void startSubService(int retryNum, Boolean onTaskRemoved) {
        if (this.isSubBinderAlive() && !onTaskRemoved) {
            ...
            if (this.nextNum > 0) {
                WhiterManager.INSTANCE.getHandler().postDelayed(this.run, 100L);
            } else {
                isReTryBinding = false;
            }
        }
    ...
    private void doStartSubService() {
        ...
        try {
            Intent intent = new Intent(this, Whiter2Service.class);
    ...
    private void bindSubService() {
        try {
            this.bindService(new Intent(this, Whiter2Service.class), this.mLocalServiceConn, 1);
    ...
    private static boolean doStartMainService(Context context) {
        try {
            Intent intent = new Intent(context, Whiter1Service.class);
            intent.putExtra("isStartService", true);
    ...
    class LocalBinder extends IProcessServiceWhiter.Stub {
        LocalBinder(Whiter1Service this$0) {
        ...
        public boolean isStartedWhiter() throws RemoteException {
            return Whiter1Service.isStartService;
        }
    }

    class LocalServiceConnection implements ServiceConnection {
        LocalServiceConnection(Whiter1Service this$0) {

        }

        public void onServiceConnected(ComponentName name, IBinder service) {
            if (WhiterManager.isDebug) {
                Log.i(Whiter1Service.TAG, "主进程 LocalService 连接子进程成功5");
            }
            try {
                ...
                Exception e = var25;
                if (WhiterManager.isDebug) {
                    Log.e(Whiter1Service.TAG, "主进程 LocalService 连接子进程 fail 5e=" + e.getMessage());
                }
            }
        }
        ...
        public void onServiceDisconnected(ComponentName name) {
            if (WhiterManager.isDebug) {
                Log.e(Whiter1Service.TAG, "主进程 LocalService 子进程服务挂掉了 onServiceDisconnected5");
            }
            subBinder = null;
            Whiter1Service.isReTryBinding = true;
            startSubService(2, false);
        }

        public void onBindingDied(ComponentName name) {
            if (WhiterManager.isDebug) {
                Log.e(Whiter1Service.TAG, "主进程 LocalService 子进程服务挂掉了 onBindingDied5");
            }
