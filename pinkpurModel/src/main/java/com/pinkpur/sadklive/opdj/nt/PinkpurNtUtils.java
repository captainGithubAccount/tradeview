...
public class PinkpurNtUtils {

    public PinkpurNtUtils() {
    }

    public static boolean isNotificationEnabled() {
        boolean isEnable = true;
        if (VERSION.SDK_INT >= 26) {
            if (isEnableV26(PinkpurManager.mContext)) {
                isEnable = true;
            } else {
                isEnable = false;
            }
    ...
    public static boolean isOngoingServiceRunning(Class serviceClass) {
        ActivityManager manager = (ActivityManager) PinkpurManager.mContext.getSystemService(Context.ACTIVITY_SERVICE);
