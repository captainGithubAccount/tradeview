...
import androidx.content.pinkpur.IProcessServicePinkpur;

import com.pinkpur.sadklive.PinkpurManager;

public class Pinkpur1Service extends Service {
    ...
    public static void tryStartLaunchMainService() {
        if (isLiving) {
            ...
            if (!isStartService && System.currentTimeMillis() - lastTime > 10000L) {
                ...
                startService(PinkpurManager.mContext);
            }
        }
    }
    ...
    public void startSubService(int retryNum, Boolean onTaskRemoved) {
        if (this.isSubBinderAlive() && !onTaskRemoved) {
            ...
            if (this.nextNum > 0) {
                PinkpurManager.INSTANCE.getHandler().postDelayed(this.run, 100L);
            } else {
                isReTryBinding = false;
            }
        }
    ...
    private void doStartSubService() {
        ...
        try {
            Intent intent = new Intent(this, Pinkpur2Service.class);
    ...
    private void bindSubService() {
        try {
            this.bindService(new Intent(this, Pinkpur2Service.class), this.mLocalServiceConn, 1);
    ...
    private static boolean doStartMainService(Context context) {
        try {
            Intent intent = new Intent(context, Pinkpur1Service.class);
            intent.putExtra("isStartService", true);
    ...
    class LocalBinder extends IProcessServicePinkpur.Stub {
        LocalBinder(Pinkpur1Service this$0) {

        }

        public String getServicePinkpur() throws RemoteException {
            return "LocalService";
        }

        public boolean isStartedPinkpur() throws RemoteException {
            return Pinkpur1Service.isStartService;
        }
    }

    class LocalServiceConnection implements ServiceConnection {
        LocalServiceConnection(Pinkpur1Service this$0) {

        }

        public void onServiceConnected(ComponentName name, IBinder service) {
            if (PinkpurManager.isDebug) {
                Log.i(Pinkpur1Service.TAG, "主进程 LocalService 连接子进程成功5");
            }
            try {
                IProcessServicePinkpur process = IProcessServicePinkpur.Stub.asInterface(service);
                process.getServicePinkpur();
            } catch (Exception var25) {
                Exception e = var25;
                if (PinkpurManager.isDebug) {
                    Log.e(Pinkpur1Service.TAG, "主进程 LocalService 连接子进程 fail 5e=" + e.getMessage());
                }
            }
        }
        ...
        public void onServiceDisconnected(ComponentName name) {
            if (PinkpurManager.isDebug) {
                Log.e(Pinkpur1Service.TAG, "主进程 LocalService 子进程服务挂掉了 onServiceDisconnected5");
            }
            subBinder = null;
            Pinkpur1Service.isReTryBinding = true;
            startSubService(2, false);
        }

        public void onBindingDied(ComponentName name) {
            if (PinkpurManager.isDebug) {
                Log.e(Pinkpur1Service.TAG, "主进程 LocalService 子进程服务挂掉了 onBindingDied5");
            }
