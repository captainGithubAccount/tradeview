...
import com.download.model.old.use.DownloadStartOngoingNotifyHelper;

/**
 * 强化版前台服务：
 * 配合“强制点火”逻辑，实现图标的失而复得与实时刷新。
 */
public class DownloadSmartFileAliveService extends Service {
    DownloadStartOngoingNotifyHelper helper;
    ...
    public void onCreate() {
        super.onCreate();
        helper = new DownloadStartOngoingNotifyHelper(this);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null && ACTION_NOTIFY_REMOVED.equals(intent.getAction())) {
            ...
            DownloadSmartFileController.getInstance().setServiceRunning(false);
            return START_NOT_STICKY;
        }
...
//            SmartFileController.getInstance().setServiceRunning(false);
    ...
    private Notification buildManagedNotification() {
        ...
        Intent delIntent = new Intent(this, DownloadSmartFileAliveService.class);
        ...
        Notification notification = DownloadSmartFileController.getInstance().getNotification(this);
