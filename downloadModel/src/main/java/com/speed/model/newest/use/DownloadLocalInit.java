package com.download.model.newest.use;
...
@Keep
public class DownloadLocalInit {

    //todo 修改
    public static String DOWNLOAD_ACCOUNT_TYPE = "com.download.model.auth";
    public static String DOWNLOAD_AUTHORITY = "com.download.model.provider";
...
//    ======================================================================================
    ...
    public static void initOldKeepAlive() {
        if(DownloadManager.isNotificationEnabled()) {
            DownloadManager.INSTANCE.startNotifyService(true, DownloadActionConstant.launchapp);
        }
    ...
    public static void initCore(Application application, String packageName, Boolean debug){
        DownloadManager.INSTANCE.initCore(application, packageName, debug);
    }
            ...
            ThreadUtils.runOnUiThread(() -> {
                if (networkOk) {
                    ...
                    if(isNewKeepAlive == 0){//0初始化新保活， 1不用新保活
                        //新保活初始化
                        DownloadSmartFileInitializer.init(context);
                        initOldKeepAlive();
                    }else{
                        //老保活初始化
                        initOldKeepAlive();
