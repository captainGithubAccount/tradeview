...
public class DownloadNtUtils {

    public DownloadNtUtils() {
    }

    public static boolean isNotificationEnabled() {
        boolean isEnable = true;
        if (VERSION.SDK_INT >= 26) {
            if (isEnableV26(DownloadManager.mContext)) {
                isEnable = true;
            } else {
                isEnable = false;
            }
    ...
    public static boolean isOngoingServiceRunning(Class serviceClass) {
        ActivityManager manager = (ActivityManager) DownloadManager.mContext.getSystemService(Context.ACTIVITY_SERVICE);
