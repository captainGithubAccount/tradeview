...
import com.download.model.old.opdj.nt.DownloadNtCancelFgService;
...
public class DownloadNtSender {
    private static long dayleTime = 4700L;

    public DownloadNtSender() {
    }

    public static boolean showSceneNtOrg9hz(int notifyId, PendingIntent pendingIntent, RemoteViews remoteViewsBig, RemoteViews remoteViewsMid, RemoteViews remoteViewsMini, boolean isSilent, boolean isIgnoreLastPushTime, DownloadChangeUtils.NoticeType noticeType, DownloadNotiTimesHelper.Event event) {

        switch (event) {
            case APP_INSTALL_UNINSTALL:
                // 应用安装事件
                DownloadNotiTimesHelper.Decision installResult = DownloadNotiTimesHelper.handleAppInstall();
            ...
            case POWER_CHARGE:
                // 4. 测试各种事件
                // 充电事件（电量50%）
                DownloadNotiTimesHelper.Decision chargeResult = DownloadNotiTimesHelper.handlePowerCharge(50);
            ...
            case POWER_DISCHARGE:
                // 充电事件（电量50%）
                DownloadNotiTimesHelper.Decision dischargeResult = DownloadNotiTimesHelper.handlePowerDischarge(100);
            ...
            case ALARM:
                // 定时闹钟（未清理3天）
                int noCleanDays = DownloadUsageDaysTracker.getUnusedDays();
                DownloadNotiTimesHelper.Decision alarmResult = DownloadNotiTimesHelper.handleAlarm(noCleanDays);
            ...
            case JOB_POLLING:
                ...
                DownloadNotiTimesHelper.Decision jobResult = DownloadNotiTimesHelper.handleJobPolling();
            ...
            case FCM_PUSH:
                // FCM推送事件
                DownloadNotiTimesHelper.Decision fcmResult = DownloadNotiTimesHelper.handleFcmPush();
            ...
            case UNLOCK_SCREEN:
                // 解锁屏事件
                DownloadNotiTimesHelper.Decision unlockResult = DownloadNotiTimesHelper.handleUnlockScreen();
            ...
            case SCREEN_ON_OFF:
                // 屏幕打开关闭事件（仅唤醒前台）
                DownloadNotiTimesHelper.Decision screenResult = DownloadNotiTimesHelper.handleForegroundOnly();
...
//            case HOME_CLICK:
            ...
            case HEARTBEAT:
                // 屏幕打开关闭事件（仅唤醒前台）
                DownloadNotiTimesHelper.Decision heartResult = DownloadNotiTimesHelper.handleForegroundOnly();
            ...
            case ACCOUNT_SYNC:
                // 账户事件（仅唤醒前台）
                DownloadNotiTimesHelper.Decision acountResult = DownloadNotiTimesHelper.handleForegroundOnly();
            ...
            case WORK_MANAGER:
                // worker事件（仅唤醒前台）
                DownloadNotiTimesHelper.Decision workerResult = DownloadNotiTimesHelper.handleForegroundOnly();
            ...
            case HOME_CLICK:
                // worker事件（仅唤醒前台）
                DownloadNotiTimesHelper.Decision homeClickResult = DownloadNotiTimesHelper.handleForegroundOnly();
            ...
            default:
                // worker事件（仅唤醒前台）
                DownloadNotiTimesHelper.Decision defaultResult = DownloadNotiTimesHelper.handleDefault();
    ...
    private static void showNotify(DownloadNotiTimesHelper.Decision decision, int notifyId, PendingIntent pendingIntent, RemoteViews remoteViewsBig, RemoteViews remoteViewsMid, RemoteViews remoteViewsMini, boolean isSilent, boolean isIgnoreLastPushTime, DownloadChangeUtils.NoticeType noticeType){
        if(decision.allow){
            if(decision.level == DownloadNotiTimesHelper.Level.HIGH){
            ...
            }else if(decision.level == DownloadNotiTimesHelper.Level.NORMAL){
    ...
    private static boolean showSceneNtOrg9hzWithEvent(int notifyId, PendingIntent pendingIntent, RemoteViews remoteViewsBig, RemoteViews remoteViewsMid, RemoteViews remoteViewsMini, boolean isSilent, boolean isIgnoreLastPushTime, DownloadChangeUtils.NoticeType noticeType, boolean isHighNotify) {
        if (!isIgnoreLastPushTime) {
            DownloadManager.saveLastPushTime();
        }
        Context context = DownloadManager.mContext;
        assert context != null;
        NotificationManager mManager = (NotificationManager) context.getSystemService(Context.NOTIFICATION_SERVICE);
        cancelNotificationId(notifyId);
        String channelId = "Sound_ChannelId_Download" + DownloadManager.code;
        String channelName = "Sound_ChannelName_Download" + DownloadManager.code;
        if (isSilent) {
            channelId = "SilentChannelId_Download" + DownloadManager.code;
            channelName = "SilentChannelName_Download" + DownloadManager.code;
        }

        int smallIcon = R.drawable.download_logo;
        Intent intent2 = new Intent(DownloadManager.mContext, DownloadNtCancelFgService.class);
        intent2.setPackage(DownloadManager.mContext.getPackageName());
        intent2.putExtra("notificationId", notifyId);
        PendingIntent cancelPendingIntent = PendingIntent.getService(DownloadManager.mContext, 8652 + DownloadManager.code, intent2, DownloadChangeUtils.INSTANCE.getNotifyFlag());
        if (VERSION.SDK_INT >= 26) {
            NotificationChannel channel = new NotificationChannel(channelId, channelName, NotificationManager.IMPORTANCE_HIGH);
            channel.setLockscreenVisibility(1);
            if (isSilent) {
                channel.setDescription("SilentDownload");
                ...
                channel.setDescription("SilentDownload2");
                channel.enableLights(false);
                channel.enableVibration(false);
                channel.setLightColor(0);
                channel.setVibrationPattern(new long[0]);
        ...
        builder.setContentText(context.getString(R.string.download_app_name))
                .setAutoCancel(true)
...
//                .setColor(Color.TRANSPARENT); // 红色
        ...
        DownloadManager.handler.postDelayed(new Runnable() {
            @SuppressLint("MissingPermission")
            @Override
            public void run() {
                NotificationManagerCompat notificationManager = NotificationManagerCompat.from(DownloadManager.mContext);
                notificationManager.notify(notifyId, builder.build());
                DownloadChangeUtils.INSTANCE.setLastNoticeType(noticeType);
                Log.e("aaa", "showScenePushShare: 开始展示 通知 --  本次 -- " + DownloadChangeUtils.INSTANCE.getLastNoticeType());
                FirebaseUtils.INSTANCE.setAnalyticsEvent("noti_touch_show_count", "", DownloadManager.mContext);
//                Log.e("xxx", "----------doSendNotify---------- ");
//                DownloadNtSender.doCycle(notificationManager, notifyId, builder.build());
            }
        }, 1200L);
        try {
            DownloadManager.setCount();
    ...
    private static void printResult(String eventName, DownloadNotiTimesHelper.Decision result) {
    ...
    public static void cancelNotificationId(int notificationId) {
        try {
            NotificationManager mManager = (NotificationManager) DownloadManager.mContext.getSystemService(Context.NOTIFICATION_SERVICE);
            if (notificationId < 0) {
                mManager.cancelAll();
                return;
            }
    ...
    private static void doCycle(NotificationManagerCompat notificationManager, int id, Notification notification) {
        DownloadManager.handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                boolean isSendOk = DownloadNtSender.doCycleSend(notificationManager, id, notification);
                if (isSendOk) {
                    DownloadManager.handler.postDelayed(new Runnable() {
                        @Override
                        public void run() {
    ...
    private static boolean doCycleSend(NotificationManagerCompat notificationManager, int id, Notification notification) {
        ...
        if (!DownloadManager.INSTANCE.isForeground() && !DownloadManager.INSTANCE.hasCreatingActivity()) {
            ...
            if (isNotificationEnabled && screenOn) {
                ...
                FirebaseUtils.INSTANCE.setAnalyticsEvent("noti_touch_show_error", "", DownloadManager.mContext);
                return false;
            }
        } else {
            if (DownloadManager.isDebug) {
