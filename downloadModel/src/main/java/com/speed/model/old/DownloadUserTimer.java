...
public class DownloadUserTimer {

    public DownloadUserTimer() {
    }

    public static void firstIn() {
        if (!DownloadSPUtils.getBoolean("firstin_app_start", false)) {
            DownloadSPUtils.putBoolean("firstin_app_start", true);
            DownloadSPUtils.putLong("firstin_app_start_time", System.currentTimeMillis());
        }

    }

    public static long getFirstInTime() {
        return DownloadSPUtils.getLong("firstin_app_start_time", 0L);
    }
